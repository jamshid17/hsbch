...
export default function AssignPage() {
  ...
  useEffect(() => {
    ...
    Object.entries(assignments).forEach(([itemId, ids]) => {
      byName[itemId] = Array.from(ids).map((id) => idToName[id]).filter(Boolean) as string[];
    });
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => { storage.clearAssignments(sessionId!); navigate(`/summary/${sessionId}`); },
    onError: (e: unknown) => setError(e instanceof Error ? e.message : t("assign.failedSave")),
  });
  ...
  function assignedCount(itemId: string) { return assignments[itemId]?.size ?? 0; }
  ...
  return (
    <div className="page">
      <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
        <h1>{t("assign.title")}</h1>
        <button onClick={() => navigate(`/people/${sessionId}`)}
      ...
      {items?.map((item) => {
        ...
        return (
          <motion.div key={item.id} className="card"
            ...
            onClick={() => setActiveItem(item)} whileTap={{ scale: 0.98 }}
          >
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 600 }}>{item.name}</div>
              <div style={{ color: "var(--hint)", fontSize: 13 }}>{item.quantity} {item.unit} × {item.price}</div>
            </div>
        ...
        {activeItem && (
          <>
            <motion.div className="overlay"
              ...
              transition={{ duration: 0.2 }} onClick={() => setActiveItem(null)}
            />
            ...
            >
              ...
              <p style={{ color: "var(--hint)", fontSize: 13, marginTop: -4 }}>{t("assign.sheetSubtitle")}</p>
              {people?.map((person: PersonOut) => {
                const checked = assignments[activeItem.id]?.has(person.id) ?? false;
                return (
                  <motion.div key={person.id} className="check-row"
                    onClick={() => togglePerson(activeItem.id, person.id)} whileTap={{ scale: 0.98 }}
                  >
